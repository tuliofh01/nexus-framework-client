...
import com.nexus.framework.core.model.NexusConfigJson
...
class ProjectGenerator(
    ...
    fun generate(
        ...
        writeAttributionNotice(projectRoot, spec, onProgress)
    ...
    fun loadTemplateBlueprint(appType: com.nexus.framework.core.model.AppType): com.nexus.framework.core.model.BlueprintFile {
    ...
    fun loadTemplateFlows(appType: com.nexus.framework.core.model.AppType): com.nexus.framework.core.model.FlowsFile {
        val path = repoRoot.resolve(TEMPLATE_DIR)
            .resolve(appType.templateFolder)
    ...
    /** Required attribution under the Nexus License — keep with every generated app. */
    private fun writeAttributionNotice(
        projectRoot: Path,
        spec: ProjectSpec,
        onProgress: (String) -> Unit,
    ) {
        val notice = """
            |NOTICE — Attribution required under the Nexus License (Nexus-1.0)
            |
            |This application ("${spec.projectName}") was generated with / derived from:
            |
            |  ${NexusBranding.FRAMEWORK_NAME}
            |  ${NexusBranding.REPO_URL}
            |  Copyright (c) Túlio Horta (GitHub: @tuliofh01)
            |
            |You MUST retain this notice (or an equivalent user-visible credit) when you
            |distribute this Derived Work.
            |
            |Non-commercial use (personal / hobby / non-commercial institutional) is
            |allowed with this attribution.
            |
            |Through 2041-07-21, prior written authorization from @tuliofh01 is required
            |if this Derived Work: (1) produces revenue, OR (2) is used in a commercial
            |institution (company / for-profit workplace). Commercial use of the Nexus
            |Toolkit itself also requires authorization through that date.
            |After 2041-07-21 those authorization restrictions expire unless renewed;
            |attribution continues.
            |
            |License identifier for this project: ${spec.license}
            |Full terms: ${NexusBranding.REPO_URL}/blob/main/LICENSE
            |
            |NO WARRANTY. The author is not responsible for misuse or unlawful activity
            |involving this software or apps derived from it.
            |
        """.trimMargin()
        val path = projectRoot.resolve("NOTICE")
        Files.writeString(path, notice)
        onProgress("Wrote NOTICE (required attribution)")
    }
