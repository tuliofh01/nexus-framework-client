module nexus.shared.script_crypto;
...
static void sha256Update(Sha256Ctx* ctx, const uint8_t* data, size_t len) {
    for (size_t i = 0; i < len; ++i) {
        ...
        ctx->bitcount++;
        if (ctx->bitcount % 64 == 0) sha256Transform(ctx, ctx->buffer);
    }
}
...
static void sha256Final(Sha256Ctx* ctx, uint8_t hash[32]) {
    ...
    while (ctx->bitcount % 64 != 56) { pad = 0; sha256Update(ctx, &pad, 1); }
    uint8_t lenbuf[8];
    for (int i = 7; i >= 0; --i) { lenbuf[i] = static_cast<uint8_t>(bitlen & 0xff); bitlen >>= 8; }
...
std::array<uint8_t, ScriptCrypto::KEY_SIZE> ScriptCrypto::deriveKey(
    const std::string& projectName, const std::string& salt, const std::string& createdAt)
{
...
void ScriptCrypto::fillRandomNonce(uint8_t* nonce, size_t len) {
    ...
    for (size_t i = 0; i < len; ++i) nonce[i] = static_cast<uint8_t>(dist(gen));
}

void ScriptCrypto::xorStream(std::string& data, const uint8_t key[KEY_SIZE], const uint8_t nonce[NONCE_SIZE]) {
