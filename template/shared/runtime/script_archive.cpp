module nexus.shared.script_archive;
...
std::array<uint8_t, 32> runtimeKey() {
...
bool ScriptArchive::load(const std::string& path) {
    ...
    const bool encrypted = version == VERSION_V2 && (static_cast<uint8_t>(reserved[0]) & FLAG_ENCRYPTED) != 0;
    std::array<uint8_t, 16> nonce{};
    if (encrypted) std::memcpy(nonce.data(), reserved + 1, 16);

    std::array<uint8_t, 32> key{};
    if (encrypted && script_protection::ENABLED) key = runtimeKey();
...
void ScriptArchive::add(const std::string& name, const std::string& source) {
    for (auto& e : entries_) {
        if (e.name == name) { e.active = true; e.source = source; return; }
    }
...
bool ScriptArchive::remove(const std::string& name) {
    for (auto& e : entries_) {
        if (e.name == name) { e.active = false; return true; }
    }
    return false;
}

bool ScriptArchive::contains(const std::string& name) const {
    for (const auto& e : entries_) { if (e.name == name && e.active) return true; }
    return false;
}

bool ScriptArchive::getSource(const std::string& name, std::string& out) const {
    for (const auto& e : entries_) {
        if (e.name == name && e.active) { out = e.source; return true; }
    }
    return false;
}
...
size_t ScriptArchive::activeCount() const {
    size_t n = 0;
    for (const auto& e : entries_) { if (e.active) ++n; }
    return n;
}
