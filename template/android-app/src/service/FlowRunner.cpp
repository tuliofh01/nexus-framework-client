...
// v1 dispatches invoke steps to PlotController / stderr log only.
...
std::string substituteState(const std::string& text, int sampleCount) {
    std::string out = text;
    const std::string token = "${state.sampleCount}";
    auto pos = out.find(token);
    if (pos != std::string::npos) {
        out.replace(pos, token.size(), std::to_string(sampleCount));
    }
    return out;
}
...
bool FlowRunner::load(const std::string& configPath, const std::string& flowsPath) {
    ...
    // Parse each flow object block (v1 sample schema — one invoke step per flow).
...
void FlowRunner::runFlow(const LoadedFlow& flow) {
    if (!flow.invokeTarget.empty()) {
        const int samples = m_controller.settings().sampleCount;
        dispatchInvoke(flow.invokeTarget, substituteState(flow.invokeArg, samples));
    }
}
...
void FlowRunner::dispatchInvoke(const std::string& target, const std::string& arg) {
    if (target == "nxs.set_samples") {
        try {
            const int samples = std::stoi(arg);
