...
public:
    ...
    void addFunction(const std::string_view specId) {
        // FunctionRegistry::activate takes const std::string& and is
        // [[nodiscard]]; materialise the view and drop the optional index.
        std::ignore = m_registry.activate(std::string{specId});
    }
    ...
    void removeFunction(const std::string_view specId) {
        m_registry.deactivate(std::string{specId});
    }
