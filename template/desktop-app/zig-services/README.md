...
zig build                    # builds {{projectName}} executable + nexus_zig library
zig build run                # builds and runs {{projectName}}
...
|----------|-------------|
| `zig-out/bin/{{projectName}}` | Main application executable |
...
## CMake coexistence (Android only)

The Android template still uses CMake for NDK builds. Desktop builds use Zig exclusively.
