...
//
// Note: a trailing defaulted std::source_location parameter cannot follow a
// parameter pack (the pack would swallow it during deduction), so this logger
// deliberately keeps the plain (level, args...) shape.
template<streamable... Args>
void log(const char* level, const Args&... args) {
    auto msg = str(args...);
    std::fprintf(stderr, "[NexusBridge::%s] %s\n", level, msg.c_str());
}
...
std::unordered_map<std::string, Callback, transparent_hash, std::equal_to<>>& callbacks() {
...
// Forward declaration: python::init and lua::init below capture
// registry::call in lambdas, but the registry namespace is defined
// further down. C++ requires the name to be visible at the lambda's
// definition point, so declare it here.
...
void init(py::module_& helpers_module) {
    ...
    //   print(result)  # "0"
    // pybind11 has no "create empty module" helper — use Python's own
    // types.ModuleType, then bind functions onto it.
    py::module_ bridge_mod =
        py::module_::import("types").attr("ModuleType")("bridge").cast<py::module_>();
    bridge_mod.def("call_cpp", [](const std::string& name, const std::string& arg) -> std::string {
...
void init(sol::state& lua) {
    ...
    lua["nxs"]["bridge"]["call_cpp"] = [](const std::string& name, const std::string& arg) -> std::string {
        return registry::call(name, arg);
    };
    lua["nxs"]["bridge"]["call_python"] = [](const std::string& fn, const std::string& arg) -> std::string {
...
[[nodiscard]] auto list() -> std::vector<std::string> {
    ...
    names.reserve(std::ranges::size(callbacks()));
    std::ranges::transform(callbacks(), std::back_inserter(names),
...
[[nodiscard]] int count() { return static_cast<int>(callbacks().size()); }
