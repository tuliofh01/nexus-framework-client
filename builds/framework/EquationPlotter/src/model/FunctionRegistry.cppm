...
public:
    ...
    /// Add a user-entered equation and immediately activate it.
    [[nodiscard]] auto addExpression(std::string expression)
        -> std::size_t {
        const auto id = "expression-" + std::to_string(m_nextId++);
        FunctionSpec spec{
            id, expression, expression,
            {0.26f, 0.62f, 0.96f, 1.0f},
        };
        m_available.push_back(spec);

        PlotSeries series;
        series.spec = std::move(spec);
        std::copy(std::begin(series.spec.defaultColor),
                  std::end(series.spec.defaultColor),
