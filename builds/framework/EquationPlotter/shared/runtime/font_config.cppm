...
#include <imgui.h>
#include <cstdio>
#include <filesystem>  // probe font path before ImGui asserts on it
...
export struct FontConfig {
    ...
    [[nodiscard]] static bool loadNerdFont(ImGuiIO& io, const char* path = kNerdFontPath) noexcept {
        ...
        // ImGui's AddFontFromFileTTF *asserts* (crashes) on a missing
        // file rather than returning nullptr, so probe the path first to
        // keep this genuinely best-effort.
        ...
        // MergeMode requires an existing base font in the atlas.
