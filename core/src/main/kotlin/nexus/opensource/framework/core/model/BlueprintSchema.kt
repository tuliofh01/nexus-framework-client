...
enum class BlueprintNodeType(
    ...
    /**
     * Visual / automation node — Langflow import and flows.json steps on the canvas.
     * Runtime execution still lives in `flows/flows.json`; this type is for editor graph display.
     */
    FLOW_AUTOMATION("flow.automation", "Flow / Langflow node"),
    ;
...
object BlueprintJson {
    ...
    fun defaultDataFor(type: BlueprintNodeType, appType: AppType): JsonObject = when (type) {
        ...
        BlueprintNodeType.FLOW_AUTOMATION -> buildJsonObject {
            put("label", "Flow node")
            put("langflowType", "Unknown")
            put("role", "step")
            put("flowId", "")
        }
    }
